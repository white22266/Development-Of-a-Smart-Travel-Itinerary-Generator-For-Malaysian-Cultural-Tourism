...
$stmt = $conn->prepare("
    SELECT i.*, tp.budget, tp.budget_tier, tp.transport_type, tp.traveller_type{$partySelect}, tp.interests, tp.preferred_states, tp.trip_days AS pref_trip_days
    FROM itineraries i
...
if ($tripDays > 1 && empty($selectedHotels)) {
    $missingInfo[] = "Confirmed Hotel";
}
if (empty($allItems)) {
    $missingInfo[] = "Itinerary Places";
}
...
<body>
                ...
                <?php if (!empty($recommendedHotels)): ?>
                    <div class="card col-12">
                        ...
                        <p class="meta">
                            ...
                            <?php if ($budget > 0): ?>Estimated around RM <?php echo number_format($nightlyBudget, 0); ?>/night.<?php endif; ?>
                        </p>
    ...
    <div class="ai-chat-panel" id="aiChatPanel" aria-live="polite">
        ...
        <div class="ai-chat-body" id="aiChatBody">
            <div class="ai-msg bot" id="aiInitialMessage">Hi, I am your local AI assistant. Ask for hotel recommendations, cost checks, route explanation, or itinerary changes. I will not save hotel or route changes unless you click a confirmation button.</div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (initial && MISSING_INFO.length) {
                initial.textContent = 'This trip summary still needs: ' + MISSING_INFO.join(', ') + '. Tell me the missing details here, then confirm the detected information so route and cost can be complete.';
        ...
        function renderHotelCards(container, hotels) {
            ...
            hotels.slice(0, 1).forEach((hotel) => {
                const card = document.createElement('div');
                card.className = 'ai-pending-card ai-hotel-card';
        ...
        async function confirmTripDate(startDate, button, nextAction = null) {
            ...
            try {
                ...
                if (data.status === 'success') {
                    if (loading) loading.textContent = data.answer || 'Trip date saved. Reloading the summary...';
                    clearAiPendingCards();
                    if (nextAction) {
                    ...
                    if (loading) loading.textContent = data.answer || data.message || 'Could not save this trip date.';
                }
            } catch (e) {
                if (button) button.disabled = false;
        ...
        async function confirmTripOrigin(originName, button) {
            ...
            try {
                ...
                const data = await parseJsonResponse(resp);
                if (data.status === 'success') {
                    if (loading) loading.textContent = data.answer || 'Starting location saved. Reloading the summary...';
                    clearAiPendingCards();
                    setTimeout(() => window.location.reload(), 700);
                } else {
                    if (button) {
                    ...
                    if (loading) loading.textContent = data.answer || data.message || 'Could not save this starting location.';
                }
            } catch (e) {
                if (button) {
                    button.disabled = false;
