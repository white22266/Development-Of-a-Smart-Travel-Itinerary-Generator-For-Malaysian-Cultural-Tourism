...
// helpers
function checked($arr, $key){ return (is_array($arr) && in_array($key, $arr, true)) ? "checked" : ""; }
function selected($val, $key){ return ($val === $key) ? "selected" : ""; }
          ...
          <div class="grid">
            <div class="card col-6" style="box-shadow:none;">
              ...
              <select name="transport_type" required class="select-placeholder"
                      style="width:100%; padding:10px 12px; border-radius:12px; border:1px solid rgba(15,23,42,0.10);">
              <option value="" disabled selected<?php echo empty($oldTransport) ? 'selected' : ''; ?>>— Please choose a transport type —</option>
            ...
            <div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px;">
              ...
              <select name="preferred_states[]" multiple
                      style="width:100%; min-height:120px; padding:10px 12px; border-radius:12px; border:1px solid rgba(15,23,42,0.10);">
                <?php foreach ($stateOptions as $s): ?>
                  <option value="<?php echo htmlspecialchars($s); ?>" <?php echo checked($oldStates, $s) ? "selected" : ""; ?>>
