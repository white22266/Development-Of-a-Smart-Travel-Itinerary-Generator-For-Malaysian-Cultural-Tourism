...
// Nightly hotel selector for itinerary review.
...
(function () {
    ...
    function ensureHotelSection(){
        var target=null;
        document.querySelectorAll('.card').forEach(function(card){if(target)return;var h=card.querySelector('h3');if(h&&h.textContent.trim().toLowerCase()==='select your hotel')target=card;});
    ...
    function headerHtml(){return '<div class="hotel-review-header"><div class="hotel-review-title"><h3>Select Your Hotels</h3><p>Choose one hotel for each overnight night. Each hotel is generated near that day\'s final kept stop, because the overnight location affects the next day route.</p></div><div class="hotel-source-pill">Nightly hotel plan</div></div>';}
    ...
    function renderShell(section){
        ...
        if(nights<=0){section.innerHTML=headerHtml()+'<div class="hotel-empty-state"><strong>No hotel night required.</strong>This itinerary has only one day, so no overnight hotel selection is required.</div>';return;}
        var html=headerHtml();
        for(var night=1;night<=nights;night++){
            html+='<div class="night-hotel-block missing" id="night-hotel-block-'+night+'"><div class="night-hotel-top"><div><div class="night-hotel-title">Night '+night+' hotel</div><div class="night-hotel-sub" id="night-stop-'+night+'">Waiting for Day '+night+' final kept stop...</div></div><div class="night-selected-pill" id="night-selected-'+night+'">Not selected</div></div><div id="night-hotel-content-'+night+'"><div class="hotel-empty-state"><strong>Loading hotels...</strong></div></div></div>';
        }
        section.innerHTML=html;
    }

    function renderNightLoading(nightNo, stop, lookupPricing){
        var block=document.getElementById('night-hotel-block-'+nightNo);var content=document.getElementById('night-hotel-content-'+nightNo);var stopEl=document.getElementById('night-stop-'+nightNo);if(!block||!content)return;
        ...
        content.innerHTML='<div class="hotel-empty-state"><strong>'+(lookupPricing?'Checking live prices...':'Loading nearby hotels...')+'</strong>'+(lookupPricing?'Using SerpAPI for pricing only.':'Searching Google Places near this night\'s final stop.')+'</div>';
    }

    function renderNightEmpty(nightNo,data){
        var block=document.getElementById('night-hotel-block-'+nightNo);var content=document.getElementById('night-hotel-content-'+nightNo);if(!block||!content)return;block.classList.add('missing');
        content.innerHTML='<div class="hotel-empty-state"><strong>No hotels available for Night '+nightNo+'.</strong>'+escHtml((data&&data.message)||'No hotel results returned.')+'</div>';
    }
    ...
    function renderNightHotels(nightNo,data){
        ...
        var html='<div class="hotel-pricing-toolbar"><div class="hotel-pricing-copy"><strong>'+(livePrices?'Live pricing available':'Prices are estimates')+'</strong><span>'+(livePrices?'Cached SerpAPI price data is shown where matched.':'Click only when you want to spend SerpAPI quota for this night pricing.')+'</span></div><button type="button" class="hotel-price-btn" data-night="'+nightNo+'">'+(livePrices?'Refresh live prices':'Check live prices')+'</button></div><div class="hotel-grid">';
        ...
        if(selectedPill){var selectedHotel=selectedName(nightNo);selectedPill.textContent=selectedHotel||'Not selected';selectedPill.classList.toggle('selected',!!selectedHotel);}block.classList.toggle('missing',!selectedName(nightNo));
    }
    ...
    function updateNightlyHotelStats(){var names=[];var total=getTotalNights();for(var night=1;night<=total;night++){if(selectedName(night))names.push('N'+night+': '+selectedName(night));}var stat=document.getElementById('stat-hotel');if(stat)stat.textContent=names.length?names.join(' | '):'None';}
    ...
    function validateNightlyHotels(){var total=getTotalNights();if(total<=0)return true;for(var night=1;night<=total;night++){if(!nightlyHotelSelections[night]||!nightlyHotelSelections[night].hotel){alert('Please select a hotel for Night '+night+' before confirming.');var block=document.getElementById('night-hotel-block-'+night);if(block)block.scrollIntoView({behavior:'smooth',block:'center'});return false;}}return true;}

    function overrideConfirmReview(){
        if(typeof window.confirmReview!=='function'||window.confirmReview.__nightlyHotels)return;
        window.confirmReview=async function(){
            if(!validateNightlyHotels())return;
        ...
        window.confirmReview.__nightlyHotels=true;
    }
