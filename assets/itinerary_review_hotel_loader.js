...
(function () {
    ...
    function headerHtml(){return '<div class="hotel-review-header"><div class="hotel-review-title"><h3>Optional Hotel Suggestions</h3><p>Hotel selection is optional. Travellers may choose a suggested hotel for each overnight night, or skip this section and book their own accommodation outside the system.</p></div><div class="hotel-source-pill">Optional add-on</div></div>';}
    ...
    function renderShell(section){
        ...
        if(nights<=0){section.innerHTML=headerHtml()+'<div class="hotel-empty-state"><strong>No hotel night required.</strong>This itinerary has only one day, so overnight hotel suggestions are not needed.</div>';return;}
        var html=headerHtml();
        for(var night=1;night<=nights;night++){
            html+='<div class="night-hotel-block not-selected" id="night-hotel-block-'+night+'"><div class="night-hotel-top"><div><div class="night-hotel-title">Night '+night+' hotel suggestion</div><div class="night-hotel-sub" id="night-stop-'+night+'">Waiting for Day '+night+' final kept stop...</div></div><div class="night-selected-pill" id="night-selected-'+night+'">Optional, not selected</div></div><div id="night-hotel-content-'+night+'"><div class="hotel-empty-state"><strong>Loading hotel suggestions...</strong></div></div></div>';
        }
    ...
    function renderNightEmpty(nightNo,data){
        var block=document.getElementById('night-hotel-block-'+nightNo);var content=document.getElementById('night-hotel-content-'+nightNo);if(!block||!content)return;block.classList.add('not-selected');
    ...
    function renderNightHotels(nightNo,data){
        ...
        if(selectedPill){var selectedHotel=selectedName(nightNo);selectedPill.textContent=selectedHotel||'Optional, not selected';selectedPill.classList.toggle('selected',!!selectedHotel);}block.classList.toggle('not-selected',!selectedName(nightNo));
    }
